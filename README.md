...
| Save interval (minutes)       | `0`              | How long should the Chat Log wait before saving to disk? This is in minutes, the minimum is 1. Set to 0 to only save when paused (warning: setting to <5 will lag a lot). All values try to save during crashes, and all save on game exit.                                                                                            | `text.chatpatches.chatlogSaveInterval`    |
...
| Chat shift animation length.  | `true`           | The length of the animation (LERP) when the chat shifts to a new height, in seconds. Setting this to 0 removes the animation and the chat shifts instantly. This is best seen with Dynamic Shifting when stats change during gameplay. constant.                                                                                                                                                                                     | `text.chatpatches.dynamicChatShift`       |
