...
/**
 * Represents the ChatLog file in the
...
public class ChatLog {
    ...
    private static final Gson json = new com.google.gson.GsonBuilder()
        ...
        .registerTypeAdapter(Text.class, (InstanceCreator<Text>) type -> LiteralText.EMPTY)
    .create();
    ...
    /**
     * Deserializes the ChatLog from {@link #CHATLOG_PATH} and resolves message data from it.
     *
     * @implNote
     * <ol>
    ...
    public static void deserialize() {
        ...
        ChatPatches.LOGGER.info("[ChatLog.deserialize] Read the ChatLog containing {} messages and {} sent messages from '{}'",
    ...
    public static void serialize(boolean crashing) {
        ...
        try {
            ...
            ChatPatches.LOGGER.info("[ChatLog.serialize] Saved the ChatLog containing {} messages and {} sent messages to '{}'", data.messages.size(), data.history.size(), CHATLOG_PATH);
    ...
    public static void restore(MinecraftClient client) {
        ...
        if(data.messages.size() > 0)
            data.messages.forEach(client.inGameHud.getChatHud()::addMessage);
