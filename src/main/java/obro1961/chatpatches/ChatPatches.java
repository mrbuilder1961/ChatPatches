...
public class ChatPatches implements ClientModInitializer {
	...
	public static GameProfile lastSender = Util.NIL_SENDER;
	 ...
	 *	<li> Registers a callback on {@link ClientCommandManager#DISPATCHER} for the {@link CopyMessageCommand}
	...
	public void onInitializeClient() {
		...
		ClientPlayConnectionEvents.JOIN.register((network, packetSender, client) -> {
			...
			if(ChatLog.loaded && Util.Flags.INIT.isSet()) {
				Util.chatHud(client).getVisibleMessages().replaceAll(ln -> new ChatHudLine<>(0, ln.getText(), ln.getId()));
