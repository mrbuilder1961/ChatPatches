...
public class ChatHudMixin {
    ...
    private int moreMsgs(int oldMax) {
        ...
        return Option.MAX_MESSAGES.changed() ? Option.MAX_MESSAGES.get() : ( mem<gb*9 ? 2048 : mem<gb*13 ? 4096 : mem<gb*17 ? 8192 : 1024 );
    };
     ...
     * in {@link #injectCounter(Text, int, int, boolean, CallbackInfo)}
    ...
    public Text modifyMessage(Text m) {
        // IF something is flagged AND its not the counter correction THEN cancel
        if(flags > 0)
            return m;

        final String mStr = m.getString(); final Style mStyle = m.getStyle();
        Date now = new Date();
        boolean boundary = mStr.equals(delAll(Option.BOUNDARY_STR.get(), "(&[0-9a-fA-Fk-orK-OR])+")) && Option.BOUNDARY.get();
        String name = msgSender.name().getString();
        ...
        Text modified = Text.empty().setStyle(mStyle)
            .append(!boundary && Option.TIME.get()
                ? (config.getTimeF(now))
                    .setStyle(Style.EMPTY
                        .withHoverEvent(Option.HOVER.get() ? new HoverEvent(SHOW_TEXT, Text.of(config.getHoverF(now))) : null)
            ...
            .append( (Option.NAME_STR.changed() && !boundary && (msgSender!=null && !msgSender.equals(Util.NIL_SENDER)) && Pattern.matches("^<[a-zA-Z0-9_]{3,16}> .+", mStr))
                ? Text.empty().setStyle(mStyle)
                    .append( Text.literal(config.getNameF(name) + " ").setStyle(mStyle
                            .withHoverEvent(new HoverEvent(SHOW_ENTITY, new HoverEvent.EntityContent(EntityType.PLAYER, msgSender.uuid(), msgSender.name())))
        ...
        // saves this message to the cache ifspace is available
    ...
    public void injectCounter(Text m, int id, int tick, boolean rfrs, CallbackInfo ci) {
        ...
        if( Option.COUNTER.get() && messages.size() > 0 && !m.getString().equals(Util.getStrTextF(Option.BOUNDARY_STR.get()).getString()) ) {
            ...
            if((flags & 6) != 6 && sibs.get(1).getString().equalsIgnoreCase(lSibs.get(1).getString())) {
                ...
                for(OrderedText text : visibles) {
                    if(visibleMessages.size() > visibles.indexOf(text))
                        visibleMessages.set(visibles.indexOf(text), new ChatHudLine<>(tick, text, id));
                    else
                        visibleMessages.add(visibles.indexOf(text), new ChatHudLine<>(tick, text, id));
                }
